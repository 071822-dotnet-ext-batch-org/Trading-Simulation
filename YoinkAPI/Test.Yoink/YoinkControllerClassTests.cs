...
namespace Test.Yoink
{
    public class YoinkControllerClassTests
    {
        ...
        [Fact]
        public async Task TestingUpdateCurrentPriceAsyncReturnAllUpdatedRowsDto()
        {
            // Arrange
            AllUpdatedRowsDto expectedResultObject = helpers.fakeAllUpdatedRowsDto();
            UpdatePriceDto upDto = new UpdatePriceDto(It.IsAny<decimal>(), It.IsAny<string>());
            ...
            var fakeBl = new Mock<IYoinkBusinessLayer>();
            fakeBl.Setup(bl=>bl.UpdateCurrentPriceAsync(It.IsAny<UpdatePriceDto>(), It.IsAny<string>()))
                .ReturnsAsync(expectedResultObject);

            var TheClassBeingTested = new YoinkController(fakeBl.Object);

            TheClassBeingTested.ControllerContext.HttpContext = new DefaultHttpContext() { User = user };
            
            // Act

            var result = await TheClassBeingTested.UpdateCurrentPriceAsync(upDto);
            var okResult = result.Result as OkObjectResult;
            var resultObject = okResult?.Value as AllUpdatedRowsDto;


            // Assert
            Assert.NotNull(resultObject);
            Assert.Equal(expectedResultObject.Buys.Count, resultObject?.Buys.Count);
            Assert.Equal(expectedResultObject.Portfolios[0]?.PortfolioID, resultObject?.Portfolios[0]?.PortfolioID);
