...
namespace Test.Yoink
{
    public class YoinkControllerClassTests
    {
        ...
        /// <summary>
        /// creates a Guid, portfolioID for a new guid
        /// var Mock to create a new  Mock
        /// Gets portfolio to make a new portfolio, gathering user ID, the new object, privacy level, type, original liquid, 
        /// current investment, liquid, current total, symbols, Total NPL, date created, and date modified
        /// Gets a new var via YoinkController, making MockBL an object
        /// classcontroller becomes a controller with Http making a new Default HTTP Context
        /// getting a result from the var which waits for the portfolioID
        /// Asserts portfolio ID, classcontroller, and portfolioget
        /// /// </summary>
        /// <returns></returns>
    ...
    /// <summary>
    /// creating fakeuser as a string
    /// creating 2 new guids
    /// adding a new date and time
    /// a 2nd datetime depicting now
    /// to create the new comment, editedcomment makes testcomment
    /// with mockcomment, another comment is being made using the duids, fakeuser, and the dates
    /// mockbl creates a new mock via business layer
    /// controller creates new yoinkcontroller
    /// a user creates new claimsprincipal
    /// controller is being used for defaulthttpcontext
    /// the results finishes the edited comment
    /// all is asserted
    /// </summary>
    /// <returns></returns>
